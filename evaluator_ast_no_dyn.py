...
from rewards_train_utils_no_dyn import *
# 1. 结合好items和generation 
CWD = "/home/ubuntu/myren/SF110"
path = "/home/ubuntu/myren/SF110"
items_path = "/home/ubuntu/myren/SF110/rl_test_end.json"      # 替换为easyR1的数据集path     #替换为模型生成的path

with open(items_path,"r",encoding="utf-8") as f:
    data = json.load(f)

pred_file = os.environ.get("PRED_FILE", "/home/ubuntu/myren/llama/LLaMA-Factory/saves/Qwen2.5-1.5B-Instruct/lora/eval_qwen2.5-1.5b-instruct_sft_rl/generated_predictions.jsonl")
...
for i in range(len(data)):
    ...
    try:
        ...
        print(assertion)
        data[i]['quantified_scores'] = score_assertion_from_statement(assertion,data[i]['soot_analysis_result'], None)
        ...
        if test_ret is None or test_ret.returncode != 0:
            ...
            if isinstance(data[i]['quantified_scores'], dict):
                data[i]['quantified_scores']['total_score'] = 0 # 强行把无效断言的启发式打分清零
            print("原始代码+断言编译失败")
...
output_name = "excution_eval_qwen2.5_no_dyn.json"
